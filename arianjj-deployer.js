    ...
    async fetch(request, env, ctx) {
        ...
        if (request.method === "POST" && url.pathname === "/api/deploy") {
            try {
                ...
                if (subData.success && subData.result && subData.result.subdomain) {
                    ...
                    const newSub = `arianjj-${Math.random().toString(36).substring(2, 8)}`;
                ...
                const workerName = `arianjj-panel-${uniqueSuffix}`;
                const dbName = `arianjj-db-${uniqueSuffix}`;
                ...
                // ============================================================
                // دریافت سورس پنل از ریپوی شخصی arianjojo
                // ============================================================
                const PANEL_SOURCE_URL = "https://raw.githubusercontent.com/arianjojo/arianjj/main/arianjj.js";
                const githubRes = await fetch(PANEL_SOURCE_URL + "?t=" + Date.now());
                if (!githubRes.ok) throw new Error("خطا در دریافت سورس پنل از گیت‌هاب.");
                const arianjjCode = await githubRes.text();

                // متادیتا با نام جدید
                const metadata = {
                    main_module: "arianjj.js",
                ...
                formData.append("arianjj.js", new Blob([arianjjCode], { type: "application/javascript+module" }), "arianjj.js");
        ...
        if (request.method === "POST" && url.pathname === "/api/list-panels") {
            try {
                ...
                for (let script of scriptsData.result) {
                    if (script.id.startsWith("arianjj-panel")) {
        ...
        if (request.method === "POST" && url.pathname === "/api/get-panel-version") {
            try {
                ...
                let version = "Unknown";
                // Match CURRENT_VERSION = 'X.Y.Z-anything' or "X.Y.Z-anything"
                const varMatch = contentText.match(/CURRENT_VERSION\s*=\s*['"]([^'"]+)['"]/i);
                if (varMatch && varMatch[1]) {
                    version = "v" + varMatch[1];
                } else {
                    const spanMatch = contentText.match(/id=["']panel-version["'][^>]*>\s*v?([^<]+)\s*<\/span>/i);
                    if (spanMatch && spanMatch[1]) {
                        version = "v" + spanMatch[1].trim();
                    }
                }
                ...
                return new Response(JSON.stringify({ success: false, version: "Unknown", error: e.message }), {
                    headers: { "Content-Type": "application/json" },
                });
            }
        }

        // ===== آپدیت پنل از گیت‌هاب (arianjojo/arianjj) =====
        if (request.method === "POST" && url.pathname === "/api/do-update") {
            try {
                ...
                // دریافت آخرین سورس از ریپوی شخصی arianjojo/arianjj
                const githubRes = await fetch("https://raw.githubusercontent.com/arianjojo/arianjj/main/arianjj.js?t=" + Date.now());
                ...
                if (!bindingsData.success) throw new Error("دسترسی به bindings ناموفق بود");
                ...
                const formData = new FormData();
                formData.append("metadata", new Blob([JSON.stringify(metadata)], { type: "application/json" }));
                formData.append("arianjj.js", new Blob([newCode], { type: "application/javascript+module" }), "arianjj.js");
        ...
        if (request.method === "POST" && url.pathname === "/api/reset-password") {
            try {
                ...
                // دریافت bindings برای پیدا کردن دیتابیس D1
                const bindingsRes = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/workers/scripts/${scriptName}/bindings`, { headers });
                const bindingsData = await bindingsRes.json();
                if (!bindingsData.success) throw new Error("دسترسی به bindings ناموفق بود");

                // حذف پسورد از دیتابیس
                ...
                // آپدیت ورکر از گیت‌هاب (arianjojo/arianjj) برای restart
                const githubRes = await fetch("https://raw.githubusercontent.com/arianjojo/arianjj/main/arianjj.js?t=" + Date.now());
                ...
                const deployData = await deployRes.json();
                if (!deployData.success) {
                    throw new Error("خطا در restart کردن ورکر");
                }
                return new Response(JSON.stringify({ success: true }), {
                    headers: { "Content-Type": "application/json" },
                });
...
// ============================================================
// HTML صفحه Deployer (کاملاً اصلاح‌شده با برند arianjj)
// ============================================================
...
<head>
    ...
    <title>arianjj Panel Deployer</title>
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center justify-center p-4">
    ...
    <div id="mainCard" class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl p-8 relative overflow-hidden z-10">
        ...
        <div class="absolute -right-12 -bottom-12 w-40 h-40 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <h2 class="text-2xl font-black text-gray-900 dark:text-white mb-2">arianjj Auto Deployer</h2>
            <p class="text-sm font-medium text-gray-500 dark:text-zinc-400">نصب خودکار پنل اختصاصی arianjj روی کلودفلر</p>
        </div>

        <div class="space-y-5 relative z-10">
            <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=arianjj-Deployer-Token" target="_blank" class="flex items-center justify-center w-full py-3.5 bg-[#d94800] hover:bg-[#e35802] text-white font-bold rounded-xl text-sm transition duration-300 shadow-lg shadow-orange-500/20 border border-[#ff943d]">
                دریافت توکن کلودفلر
            </a>
<div class="mt-2 text-center mb-4">
    ...
    <div class="flex flex-wrap justify-center items-center gap-4 mt-6 z-10">
        <a href="https://t.me/Arianesma" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-sky-500 dark:hover:text-sky-400 group">
        ...
        <a href="https://t.me/Dextoken_10x" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-sky-500 dark:hover:text-sky-400 group">
            <svg class="w-5 h-5 text-sky-500 group-hover:scale-110 transition" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69a.2.2 0 00-.05-.18c-.06-.05-.14-.03-.21-.02-.09.02-1.49.94-4.22 2.79-.4.27-.76.41-1.08.4-.36-.01-1.04-.2-1.55-.37-.63-.2-1.12-.31-1.08-.66.02-.18.27-.36.74-.55 2.92-1.27 4.86-2.11 5.83-2.51 2.78-1.16 3.35-1.36 3.73-1.37.08 0 .27.02.39.12.1.08.13.19.14.27-.01.06.01.24 0 .24z"/></svg>
            Dextoken_10x
        </a>
    </div>
    ...
    <script>
        ...
        function toggleToken() {
            ...
            if (show) {
                                ...
                                if (result.success) {
                                        ...
                                        if (result.panels.length === 0) {
                                                ...
                                                result.panels.forEach(panel => {
                                                        ...
                                                        let panelUrl = "#";
                                                        if (devSub) {
                                                                panelUrl = "https://" + panel.name + "." + devSub + ".workers.dev/panel";
                                                        }
                                                        ...
                                                        panelDiv.innerHTML = '<div class="flex flex-col">' +
                                                                '<div class="flex items-center justify-between gap-2">' +
                                                                        ...
                                                                        '<span id="version-' + panel.name + '" class="text-[10px] px-2 py-0.5 bg-gray-200 dark:bg-zinc-700 text-gray-700 dark:text-zinc-300 rounded-full font-mono whitespace-nowrap">...</span>' +
                                                                '</div>' +
                                                        '</div>' +
                                                        '<div id="btn-container-' + panel.name + '" class="w-full">' +
                                                                '<div class="flex flex-wrap gap-2 pt-2">' +
                                                                        '<a href="' + panelUrl + '" target="_blank" class="flex-1 px-3 py-1.5 bg-blue-100 hover:bg-blue-200 text-blue-700 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 dark:text-blue-400 font-bold rounded-lg text-xs transition flex items-center justify-center">ورود</a>' +
                                                                        '<button data-name="' + panel.name + '" onclick="checkPanelVersion(this.dataset.name)" class="flex-1 px-3 py-1.5 bg-cyan-100 hover:bg-cyan-200 text-cyan-700 dark:bg-cyan-900/30 dark:hover:bg-cyan-900/50 dark:text-cyan-400 font-bold rounded-lg text-xs transition">بررسی نسخه</button>' +
                                                                        '<button data-name="' + panel.name + '" onclick="updateArianjjPanel(this.dataset.name)" class="flex-1 px-3 py-1.5 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 dark:bg-emerald-900/30 dark:hover:bg-emerald-900/50 dark:text-emerald-400 font-bold rounded-lg text-xs transition">آپدیت</button>' +
                                                                        '<button data-name="' + panel.name + '" onclick="resetArianjjPassword(this.dataset.name)" class="flex-1 px-3 py-1.5 bg-amber-100 hover:bg-amber-200 text-amber-700 dark:bg-amber-900/30 dark:hover:bg-amber-900/50 dark:text-amber-400 font-bold rounded-lg text-xs transition">ریست پسورد</button>' +
                                                                        '<button data-name="' + panel.name + '" onclick="deleteArianjjPanel(this.dataset.name)" class="flex-1 px-3 py-1.5 bg-red-100 hover:bg-red-200 text-red-700 dark:bg-red-900/30 dark:hover:bg-red-900/50 dark:text-red-400 font-bold rounded-lg text-xs transition">حذف</button>' +
                                                                '</div>' +
                                                        '</div>';
                ...
                // ===== بررسی نسخه پنل =====
                async function checkPanelVersion(scriptName) {
                        const token = document.getElementById('updateApiToken').value.trim();
                        const statusBox = document.getElementById('update-status');
                        const versionBadge = document.getElementById('version-' + scriptName);
                        ...
                        if (versionBadge) {
                                ...
                                versionBadge.className = 'text-[10px] px-2 py-0.5 bg-cyan-200 dark:bg-cyan-900/50 text-cyan-700 dark:text-cyan-300 rounded-full font-mono whitespace-nowrap animate-pulse';
                        }
                        ...
                        try {
                                ...
                                if (result.success) {
                                        if (versionBadge) {
                                                versionBadge.innerText = result.version;
                                                versionBadge.className = 'text-[10px] px-2 py-0.5 bg-emerald-200 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 rounded-full font-mono whitespace-nowrap';
                                        }
                                } else {
                                        if (versionBadge) {
                                                versionBadge.innerText = 'نامشخص';
                                                versionBadge.className = 'text-[10px] px-2 py-0.5 bg-gray-200 dark:bg-zinc-700 text-gray-700 dark:text-zinc-300 rounded-full font-mono whitespace-nowrap';
                                        }
                                }
                        } catch (e) {
                                if (versionBadge) {
                                        versionBadge.innerText = 'خطا';
                                        versionBadge.className = 'text-[10px] px-2 py-0.5 bg-red-200 dark:bg-red-900/50 text-red-700 dark:text-red-300 rounded-full font-mono whitespace-nowrap';
                                }
                        }
                }

                // ===== آپدیت پنل از گیت‌هاب =====
                async function updateArianjjPanel(scriptName) {
                        const token = document.getElementById('updateApiToken').value.trim();
                        const statusBox = document.getElementById('update-status');
                        ...
                        if (!confirm('آیا از آپدیت پنل ' + scriptName + ' به آخرین نسخه مطمئن هستید؟\n\nاطلاعات کاربران حفظ می‌شود اما صفحه پنل چند ثانیه غیرفعال خواهد بود.')) return;
                        ...
                        statusBox.innerText = 'در حال آپدیت ' + scriptName + ' از گیت‌هاب...';

                        if (btnContainer) {
                                btnContainer.innerHTML = '<div class="flex items-center justify-center py-3"><svg class="animate-spin h-5 w-5 text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></div>';
                        }
                        ...
                        try {
                                ...
                                if (result.success) {
                                        ...
                                        statusBox.innerText = '✅ پنل با موفقیت آپدیت شد';
                                        setTimeout(() => checkExistingPanels(), 2500);
                                } else {
                                        throw new Error(result.error);
                                }
                        } catch (e) {
                ...
                // ===== ریست پسورد پنل =====
                async function resetArianjjPassword(scriptName) {
                        const token = document.getElementById('updateApiToken').value.trim();
                        const statusBox = document.getElementById('update-status');
                        ...
                        if (!confirm('آیا از ریست پسورد پنل ' + scriptName + ' مطمئن هستید؟\n\nپسورد فعلی حذف شده و باید دوباره تنظیم شود.')) return;

                        statusBox.classList.remove('hidden');
                        statusBox.className = 'mt-4 text-center text-sm font-bold p-3 rounded-xl bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400';
                        statusBox.innerText = 'در حال ریست پسورد ' + scriptName + '...';
                        ...
                        try {
                                ...
                                if (result.success) {
                                        statusBox.className = 'mt-4 text-center text-sm font-bold p-3 rounded-xl bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400';
                                        statusBox.innerText = '✅ پسورد ریست شد. وارد پنل شوید تا پسورد جدید تنظیم کنید.';
                                } else {
                                        throw new Error(result.error);
                                }
                        } catch (e) {
                ...
                async function deleteArianjjPanel(scriptName) {
                        const token = document.getElementById('updateApiToken').value.trim();
                        const statusBox = document.getElementById('update-status');
        ...
        async function startDeploy() {
            ...
            try {
                ...
                statusText.innerText = 'در حال دریافت پنل arianjj...';
        ...
        <div id="update-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 opacity-0 pointer-events-none transition-opacity duration-200 ease-out">
                <div id="update-modal-card" class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl p-6 transform transition-all scale-95 opacity-0 duration-200 flex flex-col max-h-[85vh]">
                        <div class="flex justify-between items-center mb-6 shrink-0">
                                <h3 class="text-xl font-bold text-gray-900 dark:text-white">مدیریت پنل‌ها</h3>
