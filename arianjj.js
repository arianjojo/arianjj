...
const IPS_REPO_URL = "https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_REPO/main/ips.txt";
...
const Router = {
	...
	async handleApi(request, url, env, ctx) {
		...
		// ===== حذف کامل سیستم آپدیت =====
		// هیچ API برای بروزرسانی وجود ندارد.
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		const noise = ["# System Update Feed: OK", "# Sync Code: " + Math.random().toString(36).slice(2, 10), "# Version: " + CURRENT_VERSION, "# Description: Secure Node Configurations", ""].join("\n");
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                <h1 class="text-lg font-bold flex items-center gap-2" dir="ltr">
                    arianjj Panel 
                    <span id="panel-version" class="text-xs px-2 py-0.5 font-semibold bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 rounded-full">v1.4.9-arianjj</span>
                </h1>
                <div class="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800/80 shadow-sm flex-shrink-0 w-fit">
                    <a href="https://t.me/Arianesma" target="_blank" rel="noopener noreferrer" class="text-sky-500 hover:text-sky-600 dark:hover:text-sky-400 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="Telegram Arianesma">
                    ...
                    <a href="https://t.me/Dextoken_10x" target="_blank" rel="noopener noreferrer" class="text-sky-500 hover:text-sky-600 dark:hover:text-sky-400 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="Telegram Dextoken_10x">
                        <svg class="w-[22px] h-[22px] flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69a.2.2 0 00-.05-.18c-.06-.05-.14-.03-.21-.02-.09.02-1.49.94-4.22 2.79-.4.27-.76.41-1.08.4-.36-.01-1.04-.2-1.55-.37-.63-.2-1.12-.31-1.08-.66.02-.18.27-.36.74-.55 2.92-1.27 4.86-2.11 5.83-2.51 2.78-1.16 3.35-1.36 3.73-1.37.08 0 .27.02.39.12.1.08.13.19.14.27-.01.06.01.24 0 .24z"/>
                        </svg>
                    </a>
        ...
        function renderUsersUI(data) {
            try {
                ...
                if (cfRequests >= 90000) {
                    ...
                    const today = new Date().toISOString().split('T')[0];
                    if (localStorage.getItem('arianjj_usage_warned_date') !== today) {
                        openUsageWarning();
                    }
                } else {
                    if (reqCard) {
...
function closePathWarning() {
    ...
    card.classList.remove('opacity-100', 'scale-100');
    card.classList.add('opacity-0', 'scale-95');
    localStorage.setItem('arianjj_path_warned_' + CURRENT_VERSION, 'true');
}
function closeUsageWarning() {
    ...
    card.classList.add('opacity-0', 'scale-95');
    const today = new Date().toISOString().split('T')[0];
    localStorage.setItem('arianjj_usage_warned_date', today);
}
...
document.addEventListener('DOMContentLoaded', () => {
        if (localStorage.getItem('arianjj_path_warned_' + CURRENT_VERSION) !== 'true') {
            const modal = document.getElementById('path-warning-modal');
            const card = modal.querySelector('div');
            modal.classList.remove('opacity-0', 'pointer-events-none');
            modal.classList.add('opacity-100', 'pointer-events-auto');
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center py-12 px-4">
    <div class="w-full max-w-xl glass rounded-3xl shadow-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            <h1 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white mb-1">arianjj - وضعیت اشتراک</h1>
