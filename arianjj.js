		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				const cfHeaders = {
					...
					"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) ArianjjPanel/1.0"
				};
				if (!currentAccountId) {
				...
				if (request.method === "POST") {
					...
					if (!finalUuid) {
						const randomHex = Array.from(crypto.getRandomValues(new Uint8Array(6)))
							...
							.join("");
						const _u = () => Math.floor(Math.random() * 256).toString(16).padStart(2, "0");
					finalUuid = `${_u()}${_u()}${_u()}${_u()}-${_u()}${_u()}-4${_u()}-${(8 + Math.floor(Math.random() * 4)).toString(16)}${_u()}-${_u()}${_u()}${_u()}${_u()}${_u()}${_u()}`;
					}
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		const dynPath = encodeURIComponent("/In_Panel_Rayeghan_Ast_Va_Gheyre_Ghabele_Foroosh/" + (user.uuid ? user.uuid.split("-")[0] : "default"));
		const links = [];
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                <h1 class="text-lg font-bold flex items-center gap-2" dir="ltr">
                    arianjj Panel
                    <span id="panel-version" class="text-xs px-2 py-0.5 font-semibold bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 rounded-full">v2.0.0-arianjj</span>
                </h1>
                <div class="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800/80 shadow-sm flex-shrink-0 w-fit">
                    <a href="https://github.com/arianjojo/arianjj" target="_blank" rel="noopener noreferrer" class="text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="GitHub">
                    ...
                    <a href="https://t.me/Arianesma" target="_blank" rel="noopener noreferrer" class="text-sky-500 hover:text-sky-600 dark:hover:text-sky-400 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="Telegram">
                        <svg class="w-[22px] h-[22px] flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69a.2.2 0 00-.05-.18c-.06-.05-.14-.03-.21-.02-.09.02-1.49.94-4.22 2.79-.4.27-.76.41-1.08.4-.36-.01-1.04-.2-1.55-.37-.63-.2-1.12-.31-1.08-.66.02-.18.27-.36.74-.55 2.92-1.27 4.86-2.11 5.83-2.51 2.78-1.16 3.35-1.36 3.73-1.37.08 0 .27.02.39.12.1.08.13.19.14.27-.01.06.01.24 0 .24z"/>
                        </svg>
                    </a>
                    <a href="https://t.me/Dextoken_10x" target="_blank" rel="noopener noreferrer" class="text-emerald-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="Bot">
                    ...
                    <div class="flex flex-col gap-2 pt-4 lg:pt-0 border-t-2 lg:border-t-0 border-gray-300 dark:border-amoled-border justify-between">
                        <div class="flex flex-col flex-1">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-2 w-full">
                                ...
                                <a href="#" target="_blank" class="text-[11px] bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-2 rounded border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition font-black shadow-sm text-center whitespace-nowrap">ساخت پـروکـسـی شخصی</a>
                            </div>
...
<div id="support-modal" class="fixed inset-0 z-[105] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-red-500/50 rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <h3 class="font-black text-xl text-gray-900 dark:text-white mb-3">حمایت از arianjj</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6 leading-relaxed font-medium">
...
function setModalState(modalId, show) {
	...
	async function checkGlobalMessage() {
        try {
            ...
            if (window.arianjj_global_msg_version !== version) {
                ...
                document.getElementById('global-message-close-btn').onclick = function() {
                    ...
                    window.arianjj_global_msg_version = version;
                };
            }
        } catch (err) {}
    }
	...
	try {
        ...
        async function exportUsersBackup() {
            ...
            try {
                ...
                downloadAnchor.setAttribute("download", "arianjj_full_backup_" + dateStr + ".json");
...
const CURRENT_VERSION = '2.0.0-arianjj';
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/arianjojo/arianjj/main/arianjj.js?t=' + Date.now());
                if (!res.ok) throw new Error('Network response was not ok');
                const text = await res.text();
                const match = text.match(/const\\s+CURRENT_VERSION\\s*=\\s*['"]([^'"]+)['"]/i);
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            window.changeRefreshRate = function(val) {
                ...
                localStorage.setItem('arianjj_refresh_rate', ms);
                window.startRefreshInterval(ms);
                showToast('نرخ رفرش پـنـل تغییر کرد');
            };
            const savedRate = localStorage.getItem('arianjj_refresh_rate');
...
const WORKER_DONATE_URL = ''; // donate endpoint disabled in arianjj build
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-center">
        <a href="https://github.com/arianjojo/arianjj" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-black dark:hover:text-white group">
...
${COMMON_TOAST_HTML}
    <script>
        ...
        function getvIeesLink() {
			...
			const dynPath = encodeURIComponent("/In_Panel_Rayeghan_Ast_Va_Gheyre_Ghabele_Foroosh/" + (u.uuid ? u.uuid.split("-")[0] : "default"));
			var links = [];
            let flagEmoji = '🌐';
